...
window.startGame = () => {
    ...
    if (n === "" || s === "") { 
        const m = showModal('alert', 'بيانات ناقصة', 'الرجاء إدخال اسمك ورقم الشعبة', () => {}); 
        if(!m) alert('الرجاء إدخال اسمك ورقم الشعبة');
        return; 
    }
    
    let allQuestions = [
        ...QUEST_BANK.SimilarPolygons,
        ...QUEST_BANK.SimilarTriangles,
        ...QUEST_BANK.ProportionalParts,
        ...QUEST_BANK.SimilarElements
    ];
    ...
    if(availableQuestions.length < 30) {
        seenIds = [];
        sb.from('user_history').update({ seen_ids: [] }).eq('user_id', `${userUID}_${SUBJECT_ID}`).then();
        availableQuestions = [...allQuestions];
    }
    
    availableQuestions.sort(() => Math.random() - 0.5);
    currentRound = availableQuestions.slice(0, 30);
